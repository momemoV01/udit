...
{
    /// <summary>
    /// Finds [UditTool] handlers on demand via reflection.
        ...
        public static MethodInfo FindHandler(string command)
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                foreach (var type in types)
                {
                    ...
                    var attr = type.GetCustomAttribute<UditToolAttribute>();
                    ...
                    if (found != null)
                    {
                        UnityEngine.Debug.LogError(
                            $"[UditConnector] Duplicate tool '{command}': " +
        ...
        public static List<object> GetToolSchemas()
        {
            ...
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                foreach (var type in types)
                {
                    ...
                    {
                        UnityEngine.Debug.LogError(
                            $"[UditConnector] Duplicate tool name '{name}': " +
