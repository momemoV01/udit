...
namespace UditConnector
{
    ...
    public static class HttpServer
    {
        ...
        static void Start()
        {
            ...
            for (var attempt = 0; attempt < MAX_PORT_ATTEMPTS; attempt++)
            {
                ...
                try
                {
                    ...
                    Debug.Log($"[UditConnector] HTTP server started on port {port}");
                    return;
                }
            ...
            Debug.LogError("[UditConnector] Failed to start HTTP server — no available port");
        }
        ...
        static void Stop()
        {
            var port = s_Port;
            StopListener();
            Debug.Log($"[UditConnector] HTTP server stopped (was port {port})");
        }
