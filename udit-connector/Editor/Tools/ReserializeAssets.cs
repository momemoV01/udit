...
{
    [UditTool(Name = "reserialize", Description = "Force reserialize Unity assets. No params = entire project.")]
    public static class ReserializeAssets
    {
        ...
        public static object HandleCommand(JObject parameters)
        {
            ...
            if (paths == null || paths.Length == 0)
            {
                ...
                Debug.Log("[UditConnector] ForceReserializeAssets: entire project");
                return new SuccessResponse("Reserialized entire project");
            }

            AssetDatabase.ForceReserializeAssets(paths);
            Debug.Log($"[UditConnector] ForceReserializeAssets: {string.Join(", ", paths)}");
