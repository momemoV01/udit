...
> 간략한 개요는 [README](../README.ko.md)를 참고하세요. 이 페이지는 모든 빌트인 명령의 상세 레퍼런스입니다.
...
`--json` 응답의 `error_code` 필드는 안정적인 식별자 (`UCI-001`, `UCI-020` 등) — 영어 메시지 텍스트 파싱 대신 코드로 분기. 전체 코드 레지스트리는 [`docs/ERROR_CODES.ko.md`](./ERROR_CODES.ko.md) 참고.
