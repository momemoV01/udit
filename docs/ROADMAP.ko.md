...
- [ ] Cookbook 시나리오 ≥ 20개 — 현재 7개 (`docs/COOKBOOK.ko.md`), 사용자 패턴 보며 확장
- [ ] 최소 3개 인디 프로젝트에 프로덕션 사용 확인 — 유기적 지표, 시간 필요
- [ ] 1년간 breaking change 0건 — **v1.0 API 동결 약속** 선언됨 (README.ko.md#api-안정성)
...
| 2026-04-15 | Reconnect success rule: ≥5s 연결 + 1 frame 수신 or reload marker 수신 | 단순 "TCP open이면 success" 규칙은 "open-then-EOF in 200ms" 케이스 (Unity 크래시 중)에서 busy loop. `lastSuccessfulReadAt` + 5초 threshold로 "정말 붙었었나" 판정. `event: reload` 수신은 backoff 1s로 reset — Unity의 정상 재시작 신호. 1s → 2s → 4s cap 스케줄 |
...
- [x] **v1.0 Pre-release sweep** (D1-D8, 2026-04-16): D1 — console `--json` data를 `{entries, count}` 객체 wrap + profiler status snake_case 통일 (breaking, Connector 0.10.0). D2 — README API Stability 섹션. D3 — `docs/COOKBOOK.md` 7개 레시피. D4 — release SHA256SUMS.txt + install checksum 검증. D5 — `udit doctor` 진단 명령. D6 — CI 매트릭스 3-OS. D7 — Unity 호환성 매트릭스. D8 — 에러 메시지 감사 (변경 불필요 확인). Decision Log 참고
